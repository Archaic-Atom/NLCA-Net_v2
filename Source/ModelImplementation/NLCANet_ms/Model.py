...
class NLCANet(ModelTemplate):
    ...
    def __NetWork(self, imgL, imgR, height, width, training=True):
        with tf.variable_scope("NLCANet"):
            ...
            Info('│   └── After ExtractUnaryFeature:' + str(imgL_feature[0].get_shape()))
            Info('│   └── After ExtractUnaryFeature:' + str(imgL_feature[1].get_shape()))
            Info('│   └── After ExtractUnaryFeature:' + str(imgL_feature[3].get_shape()))
            Info('│   └── After ExtractUnaryFeature:' + str(imgL_feature[4].get_shape()))

            Info('├── Begin Build Cost Volume')
            cost_vol = BuildCostVolumeModule(imgL_feature, imgR_feature,
                                             IMG_DISPARITY, training=training)
            Info('│   └── After Cost Volume:' + str(cost_vol[0].get_shape()))
            Info('│   └── After Cost Volume:' + str(cost_vol[1].get_shape()))
            Info('│   └── After Cost Volume:' + str(cost_vol[2].get_shape()))
            Info('│   └── After Cost Volume:' + str(cost_vol[3].get_shape()))
            Info('│   └── After Cost Volume:' + str(cost_vol[4].get_shape()))
