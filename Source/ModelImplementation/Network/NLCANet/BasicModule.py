...
class BuildCostVolumeModule(object):
    ...
    def __FusionFeatureBlock(self, imgL, imgR, training=True):
        with tf.variable_scope("FusionFeatureBlock") as scope:
            imgL = Conv2DLayer(imgL, 1, 1, 16, "Conv_2", biased=True,
                               bn=False, relu=False, training=training)
            scope.reuse_variables()
            imgR = Conv2DLayer(imgR, 1, 1, 16, "Conv_2", biased=True,
                               bn=False, relu=False, training=training)
        return imgL, imgR

    def __BuildCostVolumeBlock(self, imgL, imgR, imgL_2, imgR_2, disp_num):
        with tf.variable_scope("BuildCostVolumeBlock"):
            ...
            for d in xrange(1, int(disp_num/4) + 1):
                ...
                # cat
                slice_featuresR = tf.slice(imgR_2, [0, 0, 0, 0],
                                           [-1, height, width - d, feature_num_2])
                slice_featuresR = tf.pad(slice_featuresR, paddings, "CONSTANT")
                cost = tf.concat([imgL_2, slice_featuresR], axis=3)
                concat_cost_vol.append(cost)

            cost_vol = tf.stack(cost_vol, axis=1)
            concat_cost_vol = tf.stack(concat_cost_vol, axis=1)
            cost_vol = tf.concat([cost_vol, concat_cost_vol], axis=4)
        return cost_vol
...
class MatchingModule(object):
    ...
    def __EncoderBlock(self, x, training=True):
        with tf.variable_scope("EncoderBlock"):
            ...
            shortcut = x
            # shortcut = self.__NonLocalGroupBlock(x, 1, "Non_v0",
            #                                         training = training)
            level_list.append(shortcut)

            # 1/32
            x = DownSamplingBlock(x, 3, 96, "Level_3", training=training)
            shortcut = self.__NonLocalGroupBlock(x, 3, "Non_v1",
                                                 training=training)
            # shortcut = x
            level_list.append(shortcut)
    ...
    def __NonLocalGroupBlock(self, x, num, name, training=True):
        with tf.variable_scope(name + "/NonLocalGroupBlock"):
            for i in range(num):
                x = SpaceTimeNonlocalBlock(x, "NonLocal_" + str(i), training=training)
        return x
    ...
    def __GetWeightBlock(self, batchsize, disp_num, height, width):
        disp = tf.range(1, disp_num + 1, 1.0)
...
class FusionModule(object):
    ...
    def Inference(self, x, output_cspn, imgL, seg, training=True):
        with tf.variable_scope("FusionModule"):
            ...
            x = Conv2DLayer(x, 3, 1, 1, "Conv_2", biased=False,
                            relu=False, bn=False, training=training)
            x = tf.squeeze(x, axis=3)
            x = self.__FeatureConcat(x, imgL, seg, training=training)
    ...
    def __FeatureConcat(self, x, imgL, seg, training=True):
