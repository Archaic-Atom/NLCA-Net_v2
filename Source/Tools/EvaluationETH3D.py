...
if __name__ == "__main__":
    ...
    acc, mas, total_num = MatchingAcc(a, b)
    ...
    for i in xrange(27):
        ...
        groundPath = linecache.getline('./Dataset/labellist_disp_ETH3D.txt', i+1)
        groundPath = groundPath.rstrip("\n")

        pos = groundPath.rfind('/')
        left_name = groundPath[0:pos]
        pos = left_name.rfind('/')
        name = left_name[pos + 1:]

        imgPath = './ResultImg_ETH/' + name + '.pfm'
...
#        if i % 5 != 0:
        ...
        img, _ = ReadPFM.readPFM(imgPath)
        imgGround, _ = ReadPFM.readPFM(groundPath)
        # img = np.array([[1.0, 4, 3], [4, 5, 6], [7, 8, 9]])
        # imgGround = np.array([[1.0, 2, 3], [4, 5, 6], [7, 8, 9]])

        # print img.shape

        acc_out, mas_out, total_num_out = sess.run([acc, mas, total_num],
                                                   feed_dict={a: img, b: imgGround})

        total = total + acc_out[1]
        errTotal = errTotal + mas_out
        num = num + 1
        print str(i) + ':' + str(acc_out[1]) + ',' + str(mas_out) + ', ' + str(total_num_out)
